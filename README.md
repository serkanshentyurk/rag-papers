...
The vectors are computed once and persisted, so only the papers change triggers
a rebuild; querying just loads them.
...
- **No formal retrieval evaluation.** Retrieval quality was checked by reading
  results for known questions, not measured against a labelled set (e.g.
  recall@k). This is the largest limitation: relevance is assessed by eye, not
  quantified.
- **Extraction quality varies by layout.** Single-column papers extract cleanly;
  two-column journals produce more artifacts, and retrieval quality is bounded by
  that noise. Cleaning is deliberately minimal.
...
notebooks/
└── demo.py               # step-by-step walkthrough + the three test questions
